...
async def create_pool(loop, **kw):
    """
    创建一个全局的连接池，每个HTTP请求都是从池中获得数据库连接
    连接池由全局变量__pool存储，缺省的情况下将编码charset设为utf-8,自动提交事务
    """
    logging.info('creat database connecting pool....')
...
async def select(sql, args, size=None):
    """
    封装select,使用Cursor对象执行select语句时，通过fetchall()可以拿到结果集。
...
async def execute(sql, args, autocommit=True):
    """
        封装INSTERT,UPDATE,DELETE:使用Cursor对象执行insert，update，delete语句时，
